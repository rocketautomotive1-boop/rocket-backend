...
@ApiTags('Yampi Marketplace')
@Controller('marketplace/yampi')
export class YampiController {
  ...
  @ApiOperation({ summary: 'Autenticar na Yampi' })
  ...
  @Post('auth/refresh')
  @ApiOperation({ summary: 'Renovar token da Yampi' })
  ...
  @Post('auth/validate')
  @ApiOperation({ summary: 'Validar token da Yampi' })
  ...
  @Post('products')
  @ApiOperation({ summary: 'Criar produto na Yampi' })
  ...
  async createProduct(@Headers('authorization') auth: string, @Body() product: any) {
    ...
    const productWithToken = { ...product, accessToken };
    return await this.productAdapter.createProduct(productWithToken);
  }

  @Put('products/:externalId')
  @ApiOperation({ summary: 'Atualizar produto na Yampi' })
  ...
  async updateProductImages(
    ...
    const accessToken = this.extractToken(auth);
    const dataWithToken = { ...imageData, token: accessToken };
  ...
  async updateProductTitle(
    ...
    const accessToken = this.extractToken(auth);
    const dataWithToken = { ...titleData, token: accessToken };
  ...
  async updateProductCategory(
    ...
    const accessToken = this.extractToken(auth);
    const dataWithToken = { ...categoryData, token: accessToken };
  ...
  @Get('orders')
  @ApiOperation({ summary: 'Listar pedidos da Yampi' })
  ...
  @ApiHeader({ name: 'Author ization', description: 'Bearer token' })
  async getOrders(
    @Headers('authorization') auth: string,
    @Query() params: any
  ) {
    const accessToken = this.extractToken(auth);
    const paramsWithToken = { ...params, token: accessToken };
  ...
  async getOrderDetails(
    ...
    const accessToken = this.extractToken(auth);
    const orderData = { token: accessToken, orderId };
  ...
  @Post('orders')
  @ApiOperation({ summary: 'Criar pedido na Yampi' })
  @ApiResponse({ status: 201, description: 'Pedido criado com sucesso' })
  @ApiHeader({ name: 'Authorization', description: 'Bearer token' })
  async createOrder(@Headers('authorization') auth: string, @Body() order: any) {
    const accessToken = this.extractToken(auth);
    const orderWithToken = { ...order, token: accessToken };
  ...
  async getOrderStatuses(@Headers('authorization') auth: string) {
    const accessToken = this.extractToken(auth);
    const merchantAlias = this.extractMerchantAliasFromToken(accessToken) || 'default_merchant_alias';
    return await this.orderAdapter.getOrderStatuses(merchantAlias, accessToken);
  }

  // ===== CATEGORIAS =====

  @Get('categories')
  @ApiOperation({ summary: 'Listar categorias da Yampi' })
  ...
  async getCategories(
    ...
    const accessToken = this.extractToken(auth);
    return await this.categoryAdapter.getCategories(accessToken, parentId);
  }
  ...
  async getCategoryById(
    ...
    const accessToken = this.extractToken(auth);
    const merchantAlias = this.extractMerchantAliasFromToken(accessToken) || 'default_merchant_alias';
    return await this.categoryAdapter.getCategoryById(categoryId, accessToken, merchantAlias);
  }

  @Post('categories')
  @ApiOperation({ summary: 'Criar categoria na Yampi' })
  @ApiResponse({ status: 201, description: 'Categoria criada com sucesso' })
  @ApiHeader({ name: 'Authorization', description: 'Bearer token' })
  async createCategory(@Headers('authorization') auth: string, @Body() category: any) {
    const accessToken = this.extractToken(auth);
    const categoryWithToken = { ...category, token: accessToken };
    return await this.categoryAdapter.createCategory(categoryWithToken);
  }

  @Put('categories/:categoryId')
  @ApiOperation({ summary: 'Atualizar categoria na Yampi' })
  @ApiResponse({ status: 200, description: 'Categoria atualizada com sucesso' })
  @ApiParam({ name: 'categoryId', description: 'ID da categoria' })
  @ApiHeader({ name: 'Authorization', description: 'Bearer token' })
  ...
  @ApiOperation({ summary: 'Excluir categoria na Yampi' })
  ...
  async deleteCategory(
    ...
    const accessToken = this.extractToken(auth);
    const merchantAlias = this.extractMerchantAliasFromToken(accessToken) || 'default_merchant_alias';
    return await this.categoryAdapter.deleteCategory(categoryId, accessToken, merchantAlias);
  }
  ...
  async getCategoryAttributes(
    ...
    const accessToken = this.extractToken(auth);
    const merchantAlias = this.extractMerchantAliasFromToken(accessToken) || 'default_merchant_alias';
    return await this.categoryAdapter.getCategoryAttributes(categoryId, accessToken, merchantAlias);
  }

  // ===== WEBHOOKS =====

  @Post('webhooks/configure')
  @ApiOperation({ summary: 'Configurar webhook da Yampi' })
  ...
  async configureWebhook(
    ...
    const accessToken = this.extractToken(auth);
    return await this.yampiAdapter.configureWebhook(accessToken, webhookData);
  }
  ...
  async listWebhooks(@Headers('authorization') auth: string) {
    const accessToken = this.extractToken(auth);
    return await this.yampiAdapter.listWebhooks(accessToken);
  }
  ...
  async removeWebhook(
    ...
    const accessToken = this.extractToken(auth);
    return await this.yampiAdapter.removeWebhook(accessToken, webhookId);
  }
  ...
  // Helper method to extract merchantAlias from token
  private extractMerchantAliasFromToken(token: string): string | null {
    try {
