...
export class ShopeeProductAdapter implements IMarketplaceProductAdapter, OnModuleInit {
  ...
  async getItemList(params: { offset?: number; limit?: number; status?: string }, token: any): Promise<any> {
    return this.executeWithRetry(async (currentToken) => {
      ...
      const signedParams = (await this.signer.sign('shopee', { path, timestamp, accessToken: currentToken.accessToken, shopId: currentToken.additionalData.shopId, extra: {
        offset: params.offset || 0,
  ...
  private async uploadImage(imageUrl: string, token: any): Promise<any> {
    ...
    const params = (await this.signer.sign('shopee', { path, timestamp, accessToken: token.accessToken, shopId })).params;
    try {
      let response: any
      try {
      ...
      if (isWrongSign) {
        try {
          const altParams = (await this.signer.sign('shopee', { path, timestamp })).params;
          let retry: any
          try {
            const FormData = require('form-data')
            const fileResp = await axios.get(imageUrl, { responseType: 'arraybuffer' })
  ...
  private async getImageIdsByUrl(urls: string[], token: any): Promise<string[]> {
    ...
    const params = (await this.signer.sign('shopee', { path, timestamp })).params;
    try {
  ...
  async createProduct(product: any, marketplace?: any): Promise<any> {
    return this.executeWithRetry(async (currentToken) => {
      ...
      try {
        ...
        const params = (await this.signer.sign('shopee', { path, timestamp, accessToken, shopId })).params;
  ...
  private async getAvailableLogistics(token: any): Promise<number[]> {
    ...
    const params = (await this.signer.sign('shopee', { path, timestamp, accessToken: token.accessToken, shopId: token.additionalData.shopId })).params;
  ...
  async updateProductImages(externalId: string, images: any[]): Promise<any> {
    ...
    try {
      const timestamp = Math.floor(Date.now() / 1000);
      const path = '/product/update_item';
      const params = (await this.signer.sign('shopee', { path, timestamp, accessToken: images[0].token.accessToken, shopId: images[0].token.additionalData.shopId })).params;
  ...
  async updateProductCategory(externalId: string, category: any): Promise<any> {
    ...
    try {
      const timestamp = Math.floor(Date.now() / 1000);
      const path = '/product/update_item';
      const params = (await this.signer.sign('shopee', { path, timestamp, accessToken: category['token'].accessToken, shopId: category['token'].additionalData.shopId })).params;
  ...
  async updateProductInventory(externalId: string, inventory: any): Promise<any> {
    ...
    try {
      const timestamp = Math.floor(Date.now() / 1000);
      const path = '/product/update_stock';
      const params = (await this.signer.sign('shopee', { path, timestamp, accessToken: inventory['token'].accessToken, shopId: inventory['token'].additionalData.shopId })).params;
