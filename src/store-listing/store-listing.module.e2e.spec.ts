...
/**
 * Substitui PricingModule (@Global no AppModule real) neste teste isolado — StoreListingModule
 * injeta PRICING_PORT no construtor (getAllocationProducts), mas nenhum teste aqui exercita
 * allocations/boxes, então um mock vazio basta. STOCK_QUERY_PORT/STORE_AWARE_STOCK_QUERY_PORT
 * vêm do StockModule REAL (importado abaixo, não mockado): este teste é a rede que prova que
 * StoreListingModule ↔ StockModule não voltam a formar um ciclo de instanciação real (ver
 * docs/superpowers/specs/2026-08-29-stock-store-listing-di-cycle-fix-design.md) — um mock aqui
 * mascararia exatamente esse tipo de regressão. @Global porque um módulo importado só enxerga
 * providers do módulo-pai via export, não os providers declarados direto em
 * Test.createTestingModule({ providers: [...] }).
 */
@Global()
@Module({
  providers: [{ provide: PRICING_PORT, useValue: {} }],
...
describe('StoreListingModule (e2e)', () => {
  ...
  let stockQuery: StoreAwareStockQueryPort;
  ...
  beforeAll(async () => {
    ...
    stockQuery = moduleRef.get(STORE_AWARE_STOCK_QUERY_PORT);
  ...
  describe('leitura store-aware (Fase 4, sub-projeto 3)', () => {
    ...
    it('getStoreStockSummary retorna zero (sem lançar) quando a loja não tem StoreListing pro produto', async () => {
      const summary = await stockQuery.getStoreStockSummary(PRODUCT, STORE_A);
      expect(summary).toEqual({ onHand: 0, reserved: 0, available: 0, avgCost: 0 });
    });

    it('getStoreStockByCondition/getStoreStockByLocation retornam lista vazia sem StoreListing', async () => {
      expect(await stockQuery.getStoreStockByCondition(PRODUCT, STORE_A)).toEqual([]);
      expect(await stockQuery.getStoreStockByLocation(PRODUCT, STORE_A)).toEqual([]);
    });
    ...
    it('duas lojas com estoque do mesmo produto têm saldos independentes — sem vazamento entre elas', async () => {
      ...
      const summaryA = await stockQuery.getStoreStockSummary(PRODUCT, STORE_A);
      const summaryB = await stockQuery.getStoreStockSummary(PRODUCT, STORE_B);
      ...
      const byConditionA = await stockQuery.getStoreStockByCondition(PRODUCT, STORE_A);
      expect(byConditionA).toEqual([{ condition: 'new', onHand: 5, reserved: 0 }]);
    });

    it('getStoreStockByLocation agrega por boxId, incluindo o "sem box" (null)', async () => {
      ...
      const byLocation = await stockQuery.getStoreStockByLocation(PRODUCT, STORE_A);
    ...
    it('listStoreStockMovements/getStoreStockMovementStatistics retornam vazio sem StoreListing', async () => {
      expect(await stockQuery.listStoreStockMovements(PRODUCT, STORE_A)).toEqual([]);
      expect(await stockQuery.getStoreStockMovementStatistics(PRODUCT, STORE_A)).toEqual({});
    });

    it('listStoreStockMovements/getStoreStockMovementStatistics não vazam movimentos entre lojas', async () => {
      const listingA = await port.createOrGetStoreListing(PRODUCT, STORE_A);
      const listingB = await port.createOrGetStoreListing(PRODUCT, STORE_B);
      ...
      const movementsA = await stockQuery.listStoreStockMovements(PRODUCT, STORE_A);
      const movementsB = await stockQuery.listStoreStockMovements(PRODUCT, STORE_B);
      ...
      const statsA = await stockQuery.getStoreStockMovementStatistics(PRODUCT, STORE_A);
      const statsB = await stockQuery.getStoreStockMovementStatistics(PRODUCT, STORE_B);
    ...
    it('listStoreStockMovements respeita o limit e ordena mais recente primeiro', async () => {
      const listing = await port.createOrGetStoreListing(PRODUCT, STORE_A);
      ...
      const limited = await stockQuery.listStoreStockMovements(PRODUCT, STORE_A, 2);
