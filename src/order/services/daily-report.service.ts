...
/**
 * Relatório diário de vendas (vive em order/, dono dos dados). Agrega e envia o
 * texto pronto via WHATSAPP_PORT (enfileira). order → whatsapp é dependência
 * descendente acíclica.
 */
@Injectable()
export class DailyReportService {
  ...
  async handleDailyReport(): Promise<void> {
    ...
    try {
      ...
      await this.whatsapp.enqueue({ content: message, correlationId: 'daily-report' });
      this.logger.log('Daily report enqueued');
    } catch (err) {
  ...
  private async buildReport(): Promise<string | null> {
    ...
    const varLine = (_label: string, v: number | null) =>
  ...
  private async aggregate(from: Date, to: Date): Promise<any[]> {
    return this.orderModel.aggregate([
      { $match: { status: { $in: PAID_STATUSES }, createdAt: { $gte: from, $lte: to } } },
      ...
      },
      {
        $project: {
          ...
          marginPct: {
            $cond: [{ $gt: ['$gross', 0] }, { $multiply: [{ $divide: ['$profit', '$gross'] }, 100] }, 0],
          },
          _id: 0,
        },
      },
